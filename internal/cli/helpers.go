...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'contentful-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: contentful-pp-cli auth set-token <token>"+
			"\n      or: export CONTENTFUL_MANAGEMENT_TOKEN=<your-token>"+
			"\n      Run 'contentful-pp-cli doctor' to check auth status.", err))
...
// dbExists reports whether the local SQLite database file is present at path.
// Used by Phase 3 commands so they can return a friendly "run sync first"
// message instead of the raw sqlite "out of memory" / "unable to open"
// error code that bubbles up when OpenReadOnly is called on a missing file.

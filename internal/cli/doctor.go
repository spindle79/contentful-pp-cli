...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  contentful-pp-cli doctor
  contentful-pp-cli doctor --json
  contentful-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CONTENTFUL_MANAGEMENT_TOKEN=<your-key>"
				} else {
			...
			if os.Getenv("CONTENTFUL_MANAGEMENT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CONTENTFUL_MANAGEMENT_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CONTENTFUL_MANAGEMENT_TOKEN")
			}
			switch {
			...
			tierEnvStatus := map[string]string{}
			{
				...
				set := 0
				checked++
				if os.Getenv("CONTENTFUL_DELIVERY_TOKEN") != "" {
					set++
				}
				tierEnvStatus["cda"] = fmt.Sprintf("%d/%d set", set, checked)
			}
			{
				checked := 0
				set := 0
				...
				tierEnvStatus["cma"] = fmt.Sprintf("%d/%d set", set, checked)
			}
			{
				checked := 0
				set := 0
				checked++
				if os.Getenv("CONTENTFUL_PREVIEW_TOKEN") != "" {
					set++
				}
				tierEnvStatus["cpa"] = fmt.Sprintf("%d/%d set", set, checked)
			}
			{
				checked := 0
				set := 0
				...
				tierEnvStatus["gql"] = fmt.Sprintf("%d/%d set", set, checked)
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["Content-Type"] = "application/vnd.contentful.management.v1+json"
						authHeaders["X-Contentful-User-Agent"] = "contentful-pp-cli/0.1.0"
						authHeaders["User-Agent"] = "contentful-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'contentful-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'contentful-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'contentful-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'contentful-pp-cli sync' to refresh."
	}
	return report
}

...
In local mode: searches locally synced data only.`,
  ...
  contentful-pp-cli search "error timeout"

  # Force local search only
  contentful-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  contentful-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  contentful-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'contentful-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "ai_actions":
				results, err = db.SearchAiActions(query, limit)
			case "api_keys":
				results, err = db.SearchApiKeys(query, limit)
			case "content_types":
				results, err = db.SearchContentTypes(query, limit)
			case "delivery_content_types":
				results, err = db.SearchDeliveryContentTypes(query, limit)
			case "preview_api_keys":
				results, err = db.SearchPreviewApiKeys(query, limit)
			case "roles":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAiActions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search ai_actions failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchApiKeys(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search api_keys failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchContentTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search content_types failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchDeliveryContentTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search delivery_content_types failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPreviewApiKeys(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search preview_api_keys failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {

...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  contentful-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  contentful-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"organizations", "spaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  contentful-pp-cli workflow status

  # Show status as JSON
  contentful-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("contentful-pp-cli")
			}

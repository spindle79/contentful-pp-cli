...
Run 'api <interface>' to see that interface's methods.`,
  ...
  contentful-pp-cli api

  # Show methods for a specific interface
  contentful-pp-cli api <interface-name>`,
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Hidden && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse '%s-pp-cli %s <method> --help' for details.\n", "contentful", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run '%s-pp-cli api' to list all interfaces", args[0], "contentful")
			}

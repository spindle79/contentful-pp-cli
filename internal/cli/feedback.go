...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".contentful-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONTENTFUL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONTENTFUL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "contentful-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.contentful-pp-cli/feedback.jsonl.
When ` + "`CONTENTFUL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONTENTFUL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  contentful-pp-cli feedback list
  contentful-pp-cli feedback list --limit 5
  contentful-pp-cli feedback list --json`,

...
func newTierLimiters(rateLimit float64) map[string]*cliutil.AdaptiveLimiter {
	return map[string]*cliutil.AdaptiveLimiter{
		"cda": cliutil.NewAdaptiveLimiter(rateLimit),
		"cma": cliutil.NewAdaptiveLimiter(rateLimit),
		"cpa": cliutil.NewAdaptiveLimiter(rateLimit),
		"gql": cliutil.NewAdaptiveLimiter(rateLimit),
	}
}
...
func (c *Client) baseURLForRequest() string {
	...
	case "cda":
		return strings.TrimRight("https://cdn.contentful.com", "/")
	case "cma":
		return c.BaseURL
	case "cpa":
		return strings.TrimRight("https://preview.contentful.com", "/")
	case "gql":
		return strings.TrimRight("https://graphql.contentful.com", "/")
	default:
		return c.BaseURL
	}
}
...
func (c *Client) authForRequest() (requestAuth, error) {
	switch c.requestTier {
	case "cda":
		tierValue0 := os.Getenv("CONTENTFUL_DELIVERY_TOKEN")
		if tierValue0 == "" {
			return requestAuth{In: "header", Name: "Authorization"}, nil
		}
		value := applyTierAuthFormat("Bearer {token}", map[string]string{
			"token":                     tierValue0,
			"access_token":              tierValue0,
			"delivery_token":            tierValue0,
			"CONTENTFUL_DELIVERY_TOKEN": tierValue0,
		})
		return requestAuth{Value: value, In: "header", Name: "Authorization"}, nil
	case "cma":
		tierValue0 := os.Getenv("CONTENTFUL_MANAGEMENT_TOKEN")
		if tierValue0 == "" {
			return requestAuth{In: "header", Name: "Authorization"}, nil
		}
		value := applyTierAuthFormat("Bearer {token}", map[string]string{
			"token":                       tierValue0,
			"access_token":                tierValue0,
			"management_token":            tierValue0,
			"CONTENTFUL_MANAGEMENT_TOKEN": tierValue0,
		})
		return requestAuth{Value: value, In: "header", Name: "Authorization"}, nil
	case "cpa":
		tierValue0 := os.Getenv("CONTENTFUL_PREVIEW_TOKEN")
		if tierValue0 == "" {
			return requestAuth{In: "header", Name: "Authorization"}, nil
		}
		value := applyTierAuthFormat("Bearer {token}", map[string]string{
			"token":                    tierValue0,
			"access_token":             tierValue0,
			"preview_token":            tierValue0,
			"CONTENTFUL_PREVIEW_TOKEN": tierValue0,
		})
		return requestAuth{Value: value, In: "header", Name: "Authorization"}, nil
	case "gql":
		tierValue0 := os.Getenv("CONTENTFUL_DELIVERY_TOKEN")
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "contentful-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("X-Contentful-User-Agent", "contentful-pp-cli/0.1.0")
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "contentful-pp-cli/0.1.0")
		}

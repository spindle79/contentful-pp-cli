# Contentful Printed CLI Agent Guide

This directory is a generated `contentful-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
contentful-pp-cli doctor --json
contentful-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
contentful-pp-cli which "<capability>" --json
contentful-pp-cli <command> --help
```
...
```bash
contentful-pp-cli <command> --agent
```
...
```bash
contentful-pp-cli <command> --help
contentful-pp-cli <command> --dry-run --agent
```
